...
const faqs = [
  {
    q: "What kinds of properties do you work with?",
    a: "Luxury and boutique hotels, resorts, heritage properties, standalone fine-dining restaurants and multi-outlet F&B groups. Engagements typically start at 40 keys or a single flagship outlet."
  },
  {
    q: "How long does a typical engagement run?",
    a: "A focused diagnostic runs four to six weeks. Full operational or brand transformations usually run six to twelve months, with a defined handover phase so your team owns the outcome."
  },
  {
    q: "Do you work on-site or remotely?",
    a: "Both. Discovery and training are on-site by default; strategy, financial modelling and reviews run remotely on a fixed cadence so travel cost stays proportionate."
  },
  {
    q: "How is pricing structured?",
    a: "Fixed-fee for defined scopes, retainer for ongoing advisory, and performance-linked components where the outcome is measurable — for example RevPAR or cover growth."
  },
  {
    q: "Can you help with pre-opening projects?",
    a: "Yes. Concept definition, operating budgets, recruitment, SOP writing, vendor selection and the launch runway are all part of our pre-opening practice."
  },
  {
    q: "What happens after the engagement ends?",
    a: "You keep every manual, model and dashboard we build. Most clients keep a light quarterly review retainer so standards hold once we step back."
  }
];
      ...
      <section className="mx-auto max-w-6xl px-4 py-12 sm:px-6 sm:py-20">
        <div className="grid gap-4 lg:grid-cols-3">
          {testimonials.map((t, i) => (
            <figure key={i} className="surface flex flex-col p-6 sm:p-7">
        ...
        <div className="mx-auto max-w-3xl px-4 sm:px-6">
          ...
          <div className="mt-6 w-full sm:mt-8" id="faq-accordion">
            {faqs.map((faq, index) => {
              ...
              return (
                <div key={index} className={`accordion-item border-b border-border ${isOpen ? 'open' : ''}`}>
                  <button
                    type="button"
                  ...
                  <div className="accordion-content">
                    <div>
                      <p className="pb-4 text-xs leading-relaxed text-muted-foreground sm:pb-5 sm:text-sm">{faq.a}</p>
                    </div>
                  </div>
                </div>
              );
